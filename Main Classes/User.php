...
// }
    ...
    // {
    //     try {
    //         $stmt = $this->pdo->prepare("SELECT COUNT(*) AS user_count FROM users WHERE user_type = :user_type");
    ...
    // {
    //     try {
    //         $stmt = $this->pdo->prepare("SELECT * FROM users WHERE user_type = :user_type ORDER BY id DESC LIMIT 5");
    //         $stmt->bindParam(':user_type', $user_type);
    //         $stmt->execute();
    ...
    // {
    //     try {
    //         $stmt = $this->pdo->prepare("UPDATE users SET disable_status = :disable_status WHERE id = :user_id");
